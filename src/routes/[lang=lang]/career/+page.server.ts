...
const careerSchema = z.object({
	nameCareerForm: z.string().min(3).max(60)
});

export async function load() {
	console.log(`I am the career server :)`);
...
export const actions = {
	default: async ({ request }) => {
		const footerForm = await superValidate(request, careerSchema);
			...
			<table align="center" cellpadding="0" cellspacing="0" width="600" style="border-collapse: collapse; border: 1px solid #cccccc; margin-bottom: 25px;">
				...
				<tr>
					<td bgcolor="#ffffff" style="padding: 40px 30px 40px 30px;">
						<h1>Footer Form</h1>
						<p style="font-size: 18px;"><strong>Name:</strong> ${footerForm.data.nameCareerForm}</p>
					</td>
				</tr>
				<tr>
